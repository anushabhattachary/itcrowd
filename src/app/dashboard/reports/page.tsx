    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-[#94A3B8] mt-1">Simple, actionable results compiled from your creator marketing activations.</p>
        </div>
      ...
      {/* Filter panel */}
      <div className="flex flex-col sm:flex-row gap-4 bg-[#1A1A27] border border-white/5 p-4 rounded-2xl shadow-xl">
        <div className="flex-1 flex items-center gap-2 bg-[#0D0D14] border border-white/10 rounded-xl px-3 py-2">
          <Layers size={14} className="text-[#475569]" />
          <select
            value={selectedCampaign}
            onChange={(e) => setSelectedCampaign(e.target.value)}
            className="flex-1 bg-transparent border-0 text-xs text-white focus:outline-none focus:ring-0 cursor-pointer"
          >
            <option value="All" className="bg-[#0D0D14] text-white">All Campaigns</option>
            <option value="summer" className="bg-[#0D0D14] text-white">Summer Sweat Challenge</option>
            <option value="opening" className="bg-[#0D0D14] text-white">Midtown Studio Grand Opening</option>
          </select>
        </div>

        <div className="w-full sm:w-[200px] flex items-center gap-2 bg-[#0D0D14] border border-white/10 rounded-xl px-3 py-2">
          ...
          <select
            ...
            className="flex-1 bg-transparent border-0 text-xs text-white focus:outline-none focus:ring-0 cursor-pointer"
          >
            <option value="30days" className="bg-[#0D0D14] text-white">Last 30 Days</option>
            <option value="ytd" className="bg-[#0D0D14] text-white">Year-To-Date</option>
            <option value="may" className="bg-[#0D0D14] text-white">May 2026</option>
          </select>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          ...
          <div key={i} className="bg-[#1A1A27] border border-white/5 rounded-2xl p-5 shadow-xl flex flex-col justify-between group">
            <div className="flex justify-between items-start">
              <span className="text-[10px] font-semibold text-[#94A3B8] uppercase tracking-wider">{kpi.label}</span>
            ...
            <div className="mt-4">
              ...
              <span className="text-[10px] block text-[#475569] font-medium mt-0.5">{kpi.detail}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-[#1A1A27] border border-white/5 rounded-3xl p-6 shadow-xl flex flex-col">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-lg font-bold text-white font-[family-name:var(--font-syne)]">Estimated Reach & Engagement Trends</h2>
            <p className="text-xs text-[#94A3B8] mt-0.5">Estimated performance metrics tracked over the selected timeframe</p>
          </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={reachData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <RechartsTooltip 
                ...
                contentStyle={{ backgroundColor: '#1A1A27', borderColor: '#ffffff10', borderRadius: '12px' }}
                itemStyle={{ color: '#fff', fontSize: '12px' }}
              />
              <Area type="monotone" dataKey="reach" stroke="#7C3AED" strokeWidth={2} fillOpacity={1} fill="url(#colorReach)" name="Est. Reach" />
              {/* Secondary line for engagement */}
              <Line type="monotone" dataKey="engagement" stroke="#A3E635" strokeWidth={2} dot={true} name="Engagement %" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-6 flex flex-col justify-between">
          ...
          <div className="bg-[#1A1A27] border border-white/5 rounded-3xl p-6 shadow-xl flex-1 flex flex-col justify-between">
            <div>
              ...
              <div className="mt-4 flex items-center gap-4 bg-[#0D0D14] border border-white/5 p-4 rounded-2xl">
                ...
                <div>
                  ...
                  <span className="text-xs text-[#94A3B8] block">NIL Athlete · Football</span>
                </div>
              </div>
            </div>

            <div className="mt-4 pt-4 border-t border-white/5 flex justify-between items-center text-xs">
              <span className="text-[#94A3B8]">Est. Reach Driven</span>
          ...
          <div className="bg-[#1A1A27] border border-white/5 rounded-3xl p-6 shadow-xl flex-1 flex flex-col justify-between mt-6">
            <div>
              ...
              <div className="mt-4 flex gap-3 bg-[#0D0D14] border border-white/5 p-3 rounded-2xl items-center">
        ...
        <div className="lg:col-span-7 bg-[#1A1A27] border border-white/5 rounded-3xl p-6 shadow-xl flex flex-col justify-between">
          <div>
            <h2 className="text-lg font-bold text-white font-[family-name:var(--font-syne)]">Creator Type Breakdown</h2>
            <p className="text-xs text-[#94A3B8] mt-0.5">Analyze results driven by different creator sectors</p>
            
            {/* Segment Selector Chips */}
            <div className="flex gap-2 mt-4 bg-[#0D0D14] p-1.5 rounded-xl border border-white/5">
              {[
            ...
            <div className="mt-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-[#0D0D14] border border-white/5 p-4 rounded-xl">
                  <span className="text-[10px] text-[#475569] font-bold uppercase tracking-wider block">Total Deployed</span>
                  <span className="text-lg font-bold text-white mt-1 block font-[family-name:var(--font-syne)]">{currentSegment.count} creators</span>
                </div>
                <div className="bg-[#0D0D14] border border-white/5 p-4 rounded-xl">
                  <span className="text-[10px] text-[#475569] font-bold uppercase tracking-wider block">Combined Reach</span>
                  <span className="text-lg font-bold text-white mt-1 block font-[family-name:var(--font-syne)]">{currentSegment.totalReach}</span>
                </div>
                <div className="bg-[#0D0D14] border border-white/5 p-4 rounded-xl">
                  <span className="text-[10px] text-[#475569] font-bold uppercase tracking-wider block">Avg. Engagement</span>
                  <span className="text-lg font-bold text-white mt-1 block font-[family-name:var(--font-syne)]">{currentSegment.avgEngagement}</span>
                </div>
                <div className="bg-[#0D0D14] border border-white/5 p-4 rounded-xl">
                  <span className="text-[10px] text-[#475569] font-bold uppercase tracking-wider block">{currentSegment.metricName}</span>
                  <span className="text-lg font-bold text-white mt-1 block font-[family-name:var(--font-syne)]">{currentSegment.metricVal}</span>
                </div>
              </div>

              {/* Segment Notes */}
              <p className="text-xs text-[#94A3B8] leading-relaxed italic bg-brand-purple/5 p-3 rounded-lg border border-brand-purple/10">
          ...
          <div className="pt-6 border-t border-white/5 mt-6 flex justify-between items-center text-xs">
            <span className="text-[#475569]">All values updated as of today</span>
